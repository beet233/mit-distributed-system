...
func TestInitialElection2A(t *testing.T) {
	servers := 3
	// make_config 中依次对每个 server 做了 Make，启动了 Raft
...
func TestBasicAgree2B(t *testing.T) {
	...
	for index := 1; index < iters+1; index++ {
		...
		// 在 cfg.one 中的 rf.Start(cmd) 写入 log
...
func TestFailNoAgree2B(t *testing.T) {
	...
	DPrintf("disconnect server %d", (leader+1)%servers)
	cfg.disconnect((leader + 1) % servers)
	DPrintf("disconnect server %d", (leader+2)%servers)
	cfg.disconnect((leader + 2) % servers)
	DPrintf("disconnect server %d", (leader+3)%servers)
	...
	DPrintf("reconnect server %d", (leader+1)%servers)
	cfg.connect((leader + 1) % servers)
	DPrintf("reconnect server %d", (leader+2)%servers)
	cfg.connect((leader + 2) % servers)
	DPrintf("reconnect server %d", (leader+3)%servers)
...
func TestRejoin2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	DPrintf("disconnect server %d", leader1)
	cfg.disconnect(leader1)
	...
	leader2 := cfg.checkOneLeader()
	DPrintf("disconnect server %d", leader2)
	cfg.disconnect(leader2)

	// old leader connected again
	DPrintf("reconnect server %d", leader1)
	cfg.connect(leader1)

	cfg.one(104, 2, true)

	// all together now
	DPrintf("reconnect server %d", leader2)
	cfg.connect(leader2)
